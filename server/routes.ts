...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/generate-pdf/staff-form-recruitment", requireBearerAuth, async (req, res) => {
    try {
      ...
      // Accept multiple formats to match UI behavior and provide flexibility
      if (req.body.body && req.body.body.candidates && Array.isArray(req.body.body.candidates)) {
        // Format from UI "PDF" button: { "body": { "candidates": [...] } }
        submissions = req.body.body.candidates;
      } else if (Array.isArray(req.body)) {
        // Direct array format: [...]
        submissions = req.body;
      } else if (req.body.submissions && Array.isArray(req.body.submissions)) {
        // Alternative wrapped format: { "submissions": [...] }
        submissions = req.body.submissions;
      } else if (req.body.candidates && Array.isArray(req.body.candidates)) {
        // Simple candidates format: { "candidates": [...] }
        submissions = req.body.candidates;
      } else {
        return res.status(400).json({ 
          error: "Invalid JSON data format. Expected { body: { candidates: [...] } } or array"
        });
      }
      ...
      // Transform submissions to match expected FormStaffSubmission schema format
      const transformedSubmissions = submissions.map((submission: any) => {
        return {
          ...
          // Map Excel/API field names to FormStaffSubmission field names for PDF generation
      ...
      // Generate PDF from the submissions
      const pdfBuffer = generateFormSubmissionsPDF(transformedSubmissions);
      
      // Set headers for PDF download
      const timestamp = new Date().toISOString().split('T')[0];
      const fileName = `staff_form_submissions_${timestamp}.pdf`;
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `attachment; filename="${fileName}"`);
      res.setHeader('Content-Length', pdfBuffer.length);
  ...
  app.post("/api/generate-pdf/customer-call-recruitment", requireBearerAuth, async (req, res) => {
    try {
      ...
      // Validate each customer against the schema
      ...
      // Generate PDF from the customers
      const pdfBuffer = generateCustomersPDF(validatedCustomers);
      
      // Set headers for PDF download
      const timestamp = new Date().toISOString().split('T')[0];
      const fileName = `customer_call_recruitment_${timestamp}.pdf`;
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `attachment; filename="${fileName}"`);
      res.setHeader('Content-Length', pdfBuffer.length);
  ...
  app.post("/api/generate-pdf/staff-call-recruitment", requireBearerAuth, async (req, res) => {
    try {
      ...
      // Set headers for PDF download
      const timestamp = new Date().toISOString().split('T')[0];
      const fileName = `staff_call_recruitment_${timestamp}.pdf`;
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `attachment; filename="${fileName}"`);
      res.setHeader('Content-Length', pdfBuffer.length);
