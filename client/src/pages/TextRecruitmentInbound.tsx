...
export default function TextRecruitmentInbound() {
  const [, setLocation] = useLocation();
  const [match, params] = useRoute("/:location/staffs/text/recruitment/inbound");
  ...
  const { data: apiCandidates = [], isLoading, isFetching, error, isError, refetch } = useQuery<CandidateEvaluation[]>({
    queryKey: ['/api/external/text-recruitment-inbound', location],
    queryFn: async () => {
      const response = await fetch(`/api/external/text-recruitment-inbound?location=${location}`, {
        credentials: 'include',
      });
      if (!response.ok) {
  ...
  useEffect(() => {
    if (apiCandidates) {
      ...
      claimDataset("textRecruitmentInbound", {
      ...
      if (dataSource === "api") {
        activateOwner("textRecruitmentInbound");
      }
    }
  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      const response = await fetch(`/api/external/text-recruitment-inbound/upload?location=${location}`, {
        method: 'POST',
        body: formData,
        credentials: 'include',
      });
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="text-2xl font-semibold text-foreground mb-2">
                  Text Recruitment - Inbound Candidates
                </h2>
